...
- ROS 2 logs are in `/home/nex/.ros/log`; `latest` points to the newest launch directory, which holds only `launch.log`, while per-node logs are `<node>_<pid>_<timestamp>.log` files directly in `/home/nex/.ros/log`.
...
- Intended exception: `scripts/deploy-split.sh` re-arms the robot after a verified deployment unless `LEKIWI_DISARM_ON_FAILURE=true`; keep that re-arm.
