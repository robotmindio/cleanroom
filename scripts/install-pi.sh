...
if [[ ${1:-} == --help ]]; then
  printf 'Usage: [LEKIWI_LEROBOT_VENV=/path] %s [--enable-pi5-usb-max-current]\n' "$0"
  exit 0
fi
enable_pi5_usb_max_current=false
if [[ ${1:-} == --enable-pi5-usb-max-current ]]; then
  enable_pi5_usb_max_current=true
  shift
fi
[[ $# -eq 0 ]] || die "unknown argument: $1"

if [[ $enable_pi5_usb_max_current == true ]]; then
  model=$(tr -d '\0' </proc/device-tree/model 2>/dev/null || true)
  [[ $model == *"Raspberry Pi 5"* ]] || die "--enable-pi5-usb-max-current requires a Raspberry Pi 5"
  boot_config=/boot/firmware/config.txt
  [[ -f $boot_config ]] || boot_config=/boot/config.txt
  [[ -f $boot_config ]] || die "cannot find Raspberry Pi boot config.txt"
  log "Enabling 1.6 A USB current limit in $boot_config"
  printf 'This requires a 5 V / 5 A power supply. With a lower-rated supply, use a powered USB hub instead.\n'
  as_root python3 - "$boot_config" <<'PY'
import os
import pathlib
import sys
import tempfile
