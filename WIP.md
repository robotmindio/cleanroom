...
Unresolved:
- Arm colours: source declares printed pieces yellow and servos black, but RViz 14.1.22's installed robot_link.hpp createVisualizable passes an empty material name for every visual. getVisualWithMaterial then selects the link's first visual, matching the black printed upper arm seen in the captured UI. Diagnose/fix rendering separately; no material or geometry changes were made for the operator's colour question.
...
Next:
- Astra mounting is corrected and rebuilt (LeKiwi 334bbe6). Continue with the fixed-camera optical-frame check, Gazebo command delivery and physical sensor/arm calibration once the robot is online.
